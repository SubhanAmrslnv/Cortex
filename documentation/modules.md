...
|---|---|---|
| `.cortex/` | Directory | Framework root — all hook scripts, scanners, commands, registry, and state |
| `.claude/` | Directory | Adapter layer — wires hooks to Claude Code; contains no business logic |
...
**What lives here:** 13 Markdown files — one per slash command — containing the full implementation instructions for each command.
...
**What lives here:** `cortex.config.json` — framework version (`3.0.0`) and default runtime path configuration.

**Responsible for:** Storing framework-level configuration that does not change per-project.

**Not responsible for:** Project-specific settings (those go in `.cortex/local/`).

---

### `.cortex/cache/`

**What lives here:** `project-profile.json` — generated by `session-start.sh` at the start of each Claude Code session.

**Responsible for:** Caching the detected project type, dependencies, entry points, and folder structure so `prompt-optimizer.sh` can inject this context without re-running discovery on every prompt.

**Not responsible for:** Persistent state across framework versions. This file is regenerated automatically and can be safely deleted.
...
**Not responsible for:** Anything by default — this directory starts empty. Files placed here mirror the path structure of `.cortex/core/` and override the corresponding base file.
...
**What lives here:** `settings.json` (hook event wiring) and `commands/` (thin wrapper Markdown files).

**Responsible for:** Adapter layer only — connecting Claude Code events to hook script paths using `${CORTEX_ROOT:-$HOME/.cortex}/core/hooks/...` format. The thin wrappers in `commands/` delegate to `command-runner.sh` and then to `.cortex/commands/`.

**Not responsible for:** Any hook logic, scanner logic, or command implementation. If a behavior needs to change, change it in `.cortex/`, not here.
